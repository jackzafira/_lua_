...
function Camera.load()
    ...
    Camera.scenes.main_menu.load(function(sceneName) Camera.view(sceneName) end)
    Camera.scenes.test_board_001.load()
    love.window.setMode(cfg.window.width, cfg.window.height)

end

function Camera.mousepressed(x, y, button)
    local scene = Camera.scenes[Camera.onView]
    if scene.mousepressed then scene.mousepressed(x, y, button) end
end

function Camera.mousereleased(x, y, button)
    local scene = Camera.scenes[Camera.onView]
    if scene.mousereleased then scene.mousereleased() end
end
